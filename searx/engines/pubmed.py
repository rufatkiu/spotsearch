...
def response(resp):
    ...
    for entry in eval_xpath_list(search_results, "//PubmedArticle"):
        ...
        content = extract_text(
            eval_xpath_getindex(medline, ".//Abstract/AbstractText//text()", 0, default=None),
            allow_none=True,
        )
        doi = extract_text(
            eval_xpath_getindex(medline, './/ELocationID[@EIdType="doi"]/text()', 0, default=None),
            allow_none=True,
        )
        journal = extract_text(
            eval_xpath_getindex(medline, "./Article/Journal/Title/text()", 0, default=None),
            allow_none=True,
        )
        issn = extract_text(
            eval_xpath_getindex(medline, "./Article/Journal/ISSN/text()", 0, default=None),
            allow_none=True,
        )
        authors = []
